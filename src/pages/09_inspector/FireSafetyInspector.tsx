...
import type { FSISInspectorClass } from "@/types/inspectorType";
import type { SearchStationModel } from "@/types/stationTypes";
import InspectorModal, { type InspectorFormSubmit } from "./components/inspectormodal";
import InspectorView from "./components/inspectorview";
import { exportInspectorLedger, num, rowTotal } from "./inspectorexport";
...
const TITLE = "Fire Safety Inspector";
const DESCRIPTION = "Station-level Fire Safety Inspector training capability.";
const ENTITY_LABEL = "Inspector";
const ADD_LABEL = "Add Inspector";
const TOTAL_LABEL = "Total Inspectors";
const ICON = <ShieldCheck className="h-5 w-5 text-primary" />;
...
export default function FireSafetyInspectorPage() {
  ...
  const provinceParams = React.useMemo<FSISInspectorClass[]>(() => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const resp = await inspectorAPI.getLedger(
        {
          parameters: {
            searchkey: debouncedSearch.trim(),
            provinces: JSON.parse(provinceKey) as FSISInspectorClass[],
          },
          pagenumber: page,
          pagesize: pageSize,
        },
      ...
      if (!ok) {
        toast.error(error || "Unable to load fire safety inspector records.");
        setRows([]);
        setTotal(0);
      } else {
        const list = Array.isArray(data) ? data : [];
        setRows(list.map(toInspectorRow));
  ...
  const openView = async (row: InspectorRow) => {
    ...
    const resp = await inspectorAPI.getDetail(
      { inspectorno: row.recordno },
      { suppressGlobalLoading: true },
    );
    const { ok, data } = unwrap<unknown>(resp);
    if (ok && data) setViewRow(toInspectorRow(data));
  };
  ...
  const checkExistingRecord = React.useCallback(async (stationno: string, stationname?: string) => {
    ...
    const existsResp = await inspectorAPI.CheckExist(
      { stationno: trimmed },
      { suppressGlobalLoading: true, suppressErrorToast: true },
    );
    const exists = unwrap<boolean>(existsResp);
    if (!exists.ok) {
      toast.error(exists.error || "Unable to check existing inspector record.");
      return null;
    }
    ...
    if (!ok) {
      toast.error(error || "Unable to check existing inspector record.");
      return null;
    }

    const list = (Array.isArray(data) ? data : []).map(toInspectorRow);
    const existing = list.find((row) => String(row.stationno ?? "") === trimmed);
    if (!existing?.recordno) return null;

    const detailResp = await inspectorAPI.getDetail(
      { inspectorno: existing.recordno },
      { suppressGlobalLoading: true, suppressErrorToast: true },
    );
    const detail = unwrap<unknown>(detailResp);
    if (!detail.ok || !detail.data) return toInspectorRow(existing);

    setExistingStationLabel(stationname || existing.stationname || trimmed);
    return toInspectorRow(detail.data);
  }, []);
  ...
  const handleSubmit = async (payload: InspectorFormSubmit): Promise<boolean> => {
    const resp = await inspectorAPI.create({
      inspectorno: payload.recordno || EMPTY_GUID,
      stationno: payload.stationno,
      withtrainingcount: Number(payload.values.withtrainingcount ?? 0) || 0,
      withouttrainingcount: Number(payload.values.withouttrainingcount ?? 0) || 0,
      remarks: payload.remarks,
      encodedby: user?.memberno ?? "",
    });
    const { ok, error } = unwrap(resp);
    if (!ok) {
      toast.error(error || "Unable to save the inspector record.");
      return false;
    }
    toast.success(payload.recordno ? "Inspector record updated." : "Inspector record added.");
    refresh();
    return true;
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const resp = await inspectorAPI.delete({
        inspectorno: deleteRow.recordno,
        deletedby: user?.memberno ?? "",
        roleno: Number(systemAccess?.roleno ?? 0) || 0,
      });
      const { ok, error } = unwrap(resp);
      if (!ok) {
        toast.error(error || "Unable to delete the inspector record.");
        return;
      }
      toast.success("Inspector record deleted.");
      setDeleteRow(null);
      refresh();
    } finally {
      setDeleting(false);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const { ok, data, error } = unwrap<unknown[]>(resp);
      if (!ok) {
        toast.error(error || "Unable to export fire safety inspector records.");
        return;
      }
      await exportInspectorLedger(
        (Array.isArray(data) ? data : []).map(toInspectorRow),
        FIELDS,
        TOTAL_LABEL,
        TITLE,
        {
  ...
  return (
    <div className="space-y-6">
      ...
      <SecureDeleteDialog
        ...
        description="This permanently removes the inspector record for this station."
        deleting={deleting}
