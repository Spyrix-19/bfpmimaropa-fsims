...
export interface InspectorMatrixGroup {
  provincename: string;
...
/** Exports the Fire Safety Inspector matrix, grouped by province with province totals. */
export function exportInspectorMatrix(
  groups: InspectorMatrixGroup[],
  fields: InspectorField[],
  totalLabel: string,
  title: string,
) {
  const sumOf = (list: InspectorRow[], key: string) =>
    list.reduce((sum, r) => sum + num(r, key), 0);
